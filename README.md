...
- 完整資料 JSON 備份與還原(還原前先顯示備份時間與內容);手機上分兩步:先準備好檔案,再按「儲存備份檔」叫出分享面板
  (準備好之後資料又變了,例如同步拉到別台的改動,就收回來重新準備,不會存到舊的那份)
...
- **今天完成 · 再學一點**:今天的份做完、牌組裡還有沒學過的字時,牌組頁的按鈕直接加碼一輪新卡(`?more=1`,用過就從網址拿掉,重新載入不會再多一批)。
...
- Excel 預設存的「CSV(逗號分隔)」不是 UTF-8(繁中 Windows 是 Big5、日文 Windows 是 Shift_JIS):選檔後會自動辨識並轉換,畫面上會說一聲;
  「Unicode 文字」(存成 .txt,UTF-16、tab 分隔,第一格空著也不會錯位)也選得到、認得
- 選了檔案就照檔名(.apkg 是裡面的牌組名)挑目標:已經有同名的牌組就加進那一副(重匯同一個檔案或更新過的版本只補上新的字),
  沒有就建立新牌組、名稱先填好。自己選的牌組、自己打的名稱不動;在選單裡特地選了「建立新牌組」就照辦(只算那一次匯入)
- 匯入完目標切到剛匯進去的那副,再按一次只補上新的字;換下一個檔案時照新檔名重挑,不會默默匯進上一副。
  匯入中不能換分頁;選好的牌組在別的分頁被刪掉(或同步拉到刪除)會退回「建立新牌組」,寫入前也會再確認一次
...
- 輸入的金鑰會先正規化(大小寫、空白、全形、少了連字號都接受)。舊版可以自訂金鑰(大小寫有差、照原樣存):
  ...
  先問「確定沒打錯嗎？」(預設停在「重新輸入」,手快再按一次 Enter 也不會連進去)—— 首頁的「在別台用過字卡？」、
  設定頁的輸入金鑰、合併、捨棄這台、換金鑰都一樣。
...
- 複習中同步拉到別台對畫面上這張卡的改動(那邊複習過、標成已會、刪掉)會直接換下一張(選單或編輯面板開著時等關掉再換,
  不會套到沒看過的下一張);評分時也會比對資料庫,
  不會拿畫面上的舊排程把別台的結果蓋掉。
...
- **只存這台的人輸入已有資料的金鑰**:可選「一起帶過去(合併)」或「捨棄這台,改用雲端的」。合併時空間裡已經有的設定
  (例如別台最佳化過的參數)優先,不會被這台的預設值蓋掉;這台新帶進去的牌組併進同名的那副(兩台都從同一份範本開始的話,
  那邊沒有的字搬過去,兩邊都有的字留空間裡那筆、進度留多的那份(較舊的複習不會蓋掉較新的),
  「已經會了/先不學」任一邊標過就留著),不會每個字兩份。
  空間原本就有的牌組不算這台帶進去的(停止同步後回到同一個空間,共用的那副不會被併掉);合併當下那次同步沒成功的話,
  之後第一次成功的同步會補做。
- 隔離範圍:伺服器以 id 當全部空間共用的主鍵,再以 `x-sync-space` header 過濾。同一個 id 已經屬於別的空間時,
  伺服器**不寫、不搬**,回報 `conflicts`(參照了別的空間的列也不寫,被參照的那一列一起回報);客戶端把那些列換一組新 id 再推。
  新 id 由空間和原本的 id 算出來:同一份資料在兩台各自合併進同一個空間,換出來的 id 一樣,不會變成兩份。
