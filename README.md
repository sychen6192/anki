...
- 輸入的金鑰會先正規化(大小寫、空白、全形、少了連字號都接受);連到的空間是空的會提醒「確定沒打錯嗎？」,可以退回去重打。
- 同步時機:啟動、回到前景(60 秒內去重)、每 15 分鐘、複習中每評一張(15 秒內連續評分合併成一次)、切到背景時補推
  (keepalive,頁面凍結或關掉後還能送完)、匯入或編輯後(延遲 3 秒合併)、手動。背景同步失敗會在「設定」分頁掛紅點;
  超過一天沒同步成功才在牌組頁出現提醒。錯誤訊息講人話(例如「連不上伺服器,連上網路後會自動同步;資料都還在這台」)。
...
- 隔離範圍:伺服器以 id 當全部空間共用的主鍵,再以 `x-sync-space` header 過濾。同一個 id 推進另一個空間會把那一列「搬走」,
  所以客戶端保證同一個 id 只進一個空間 —— 換空間時清空本機,或(停止同步後改用別組金鑰)換一組新 id。
  設定列的 id 是固定名稱(例如 `fsrs`),伺服器存的時候會在 id 前面加上空間,讀出來再拿掉,各空間各有一份。
- **還原備份**:還原前先顯示備份時間與內容;同步中還原時,備份之後才新增的牌組、字、卡片也會刪掉,
  讓這台、雲端和其他裝置都變回備份時的樣子(之後的複習進度會不見)。
